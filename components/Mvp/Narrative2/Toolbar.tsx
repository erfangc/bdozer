...
function ToolButton({ className, children, ...props }: React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement>) {
    return (
        <button
            className={`text-blueGray-50 px-2 py-2 text-sm font-light rounded-md focus:outline-none flex flex-col hover:bg-blueGray-600 justify-center items-center ${className} transition ease-linear`}
            {...props}
        >
            {children}
        </button>
