            ...
            component = (
                <div className="grid grid-flow-row gap-4 grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <Money
                        ...
                        running={loading}
                    />
                    <CardPercent value={stockAnalysis?.model?.terminalGrowthRate} label={"Terminal Growth Rate"}
                                 running={loading}/>
    ...
    return (
        <div>
            <div className="flex mb-4">
                <button
                    className={`text-blueGray-200 text-sm px-2 w-32 py-1.5 transition ease-out rounded-l-xl ${tab === 'cards' ? 'bg-blueGray-700' : 'bg-blueGray-800'} focus:outline-none`}
                    onClick={() => setTab('cards')}>Cards
                </button>
                <button
                    className={`text-blueGray-200 text-sm px-2 w-32 py-1.5 transition ease-out rounded-r-xl ${tab === 'chart' ? 'bg-blueGray-700' : 'bg-blueGray-800'} focus:outline-none`}
                    onClick={() => setTab('chart')}>Chart
                </button>
            </div>
            {component}
        </div>
