...
export function PublishedToggle({setPublished, published}: Props) {
    ...
    return (
        <div>
            <button
                onClick={viewAll}
                className={`focus:outline-none border-l border-t border-b border-amber-700 transition ease-linear hover:bg-amber-500 hover:text-blueGray-800 px-2 py-1 rounded-l ${published === undefined ? 'bg-amber-500 text-blueGray-800' : 'text-amber-600'}`}>
                All
            </button>
            <button
