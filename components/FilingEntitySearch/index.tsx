    ...
    return (
        <div className={`text-blueGray-50 ${props.className}`}>
            <div className="relative container max-w-lg">
                <div className={`flex items-center bg-blueGray-700 px-4 ${entities.length > 0 ? 'rounded-t' : 'rounded'}`}>
                    {
                        props.loading
                            ?
                    ...
                    <input
                        autoFocus
                        value={term}
                        placeholder={props.placeholder ?? 'Search company by ticker or name'}
                        onChange={e => changeTerm(e.currentTarget.value)}
                        className="relative text-blueGray-50 pl-4 py-4 focus:outline-none bg-blueGray-700 placeholder-blueGray-400 text-lg w-80"
                    />
                </div>
