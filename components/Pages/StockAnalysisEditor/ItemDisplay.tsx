...
export function ItemDisplay(props: Props) {
    ...
    const subtotal = item.type === ItemTypeEnum.SumOfOtherItems
    return (
        <div
            className={
                `cursor-pointer grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 rounded py-2 
                ${subtotal ? '' : 'bg-blueGray-800'} shadow-lg px-4 hover:bg-blueGray-700 transition ease-linear 
                ${orphaned ? 'ring-2 ring-rose-700' : null}`
            }
            onClick={() => router.push(`/control-panel/stock-analyses/${id}/items/${item.name}`)}
        >
            <div className="flex items-center overflow-hidden">
                <span className={subtotal ? 'font-extrabold' : ''}>
                    {item.description ?? item.name}
                </span>
            </div>
            <div className="flex justify-start md:justify-end items-center">
                <NumberFormat
                    className={subtotal ? 'font-extrabold' : ''}
                    thousandSeparator
                    decimalScale={0}
                    displayType='text'
            ...
            <div className="flex justify-start lg:justify-end items-center space-x-4">
                ...
                {subtotal && !overridden ? <Nothing/> : !overridden ? <Check/> : <Manual/>}
            </div>
        </div>
    )
}
